...
if($#ARGV<0){
	print	"Usage:ConvUnixTimeToUTC <gps_time>\n";
	exit(-1);
}
$gpstm = eval(shift(@ARGV));

InitUnixTimeToUTCConverter();

print ConvUnixTimeToUTC($gpstm);	print "\n";

#-------------------------------------------------------------------------
# Global variable for UTC to GPS time conversion
our ($secsGpsUnix, $LEAP_SECONDS, %monthToMonthId);

sub InitUnixTimeToUTCConverter
{
	# Just for Windows
...
sub ConvUnixTimeToUTC #( $unixTime)
#Example $utc
# "Nov 16, 2006 06:59:02 GMT";
# "Nov 28, 2006 	20:03:02.365 GMT";
{

	return scalar gmtime($_[0]);
}
