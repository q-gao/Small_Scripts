...
if __name__ == '__main__':      
    if len(sys.argv) < 4:
        print "USAGE: StudyMinHppHeightSkewDetectionForAllChars.py <rotation_max> <rotation_step> <font_name>"
    ...
    for ii in xrange( len(char) ):    
        c = char[ii]
        prob = GetHppSkewDedectionProbForText( c, sys.argv[3], int(sys.argv[1]), int(sys.argv[2]))
        print c, prob
        sys.stdout.flush()
